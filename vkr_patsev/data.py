...
# Это путь, по которому создаются и затем сохраняются обработанные данные, обновите его до того места, где вы хотите видеть свои данные
...
# Вот несколько знаков препинания на выбор, в зависимости от того, что вы хотите, чтобы ваша система выучила
...
CRAP_TOKENS = {
   ...
   # знаки препинания, которые не включены ни в словарь, ни в сопоставление, должны быть добавлены в CRAP_TOKENS
PAUSE_PREFIX = "<sil="
...
def write_processed_dataset(input_files, output_file):
    ...
    данные будут состоять из двух наборов выровненных подпоследовательностей (слов и знаков препинания) токенов MAX_SEQUENCE_LEN (фактически последовательность знаков препинания будет на 1 элемент короче).
    Если предложение сокращено, то оно будет полностью добавлено в следующую подпоследовательность (слова перед сокращением принадлежат обеим последовательностям).
    """
    ...
    last_eos_idx = 0  # if it's still 0 when MAX_SEQUENCE_LEN is reached, then the sentence is too long and skipped.
                      # если при достижении MAX_SEQUENCE_LEN значение по-прежнему равно 0, то предложение слишком длинное и пропущено.
    last_token_was_punctuation = True  # skipt first token if it's punctuation
                                       # пропустите первый знак, если это знак препинания
    last_pause = 0.0
    ...
    skip_until_eos = False  # if a sentence does not fit into subsequence, then we need to skip tokens until we find a new sentence
                            # если предложение не вписывается в подпоследовательность, то нам нужно пропускать токены до тех пор, пока мы не найдем новое предложение
        ...
        with codecs.open(input_file, "r", "utf-8") as text:
            ...
            for line in text:
                ...
                for token in line.split():
                    ...
                    # Сначала сопоставьте все знаки препинания с известными знаками препинания
                    ...
                    if skip_until_eos:
                        ...
                        if (
                            ...
                            # если мы столкнемся с последовательностями типа: "... ВОСКЛИЦАТЕЛЬНЫЙ ЗНАК .ТОЧКА ...", тогда мы используем только первый знак препинания и пропускаем последующие
                            continue
                        ...
                        if token in EOS_TOKENS:
                            last_eos_idx = len(
                               ...
                               # нет -1, потому что токен еще не добавлен
                    ...
                    if (
                        ...
                        # это также означает, что последний знак был словом
                        ...
                        # Предложение не вписалось в подпоследовательность - пропустите его
                        if last_eos_idx == 0:
                            skip_until_eos = True
                            ...
                            last_token_was_punctuation = True  # next sequence starts with a new sentence, so is preceded by eos which is punctuation
                                                               # следующая последовательность начинается с нового предложения, поэтому ей предшествует eos, который является знаком препинания
                        else:
                            subsequence = [
                            ...
                            # Перенести незаконченное предложение в следующую подпоследовательность
                        ...
                        last_eos_idx = 0  # sequence always starts with a new sentence
                                          # последовательность всегда начинается с нового предложения
